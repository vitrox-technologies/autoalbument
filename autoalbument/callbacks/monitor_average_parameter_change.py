...
class MonitorAverageParameterChange(Callback):
    ...
    def get_combined_parameter_values(cls, state_dict, prefixes):
        ...
        for prefix in prefixes:
            ...
            temperature = torch.zeros(weights.size(), dtype=weights.dtype).to(
                weights.device
            )
            ...
            for i, weight in enumerate(weights):
                ...
                probability = (
                    weight * state_dict[probability_key].clamp(0.0, 1.0)
                ).item()
                magnitude = state_dict.get(magnitude_key)
                if magnitude is not None:
                    magnitude = magnitude.clamp(0.0, 1.0).item()
                value = (
                    probability * magnitude if magnitude is not None else probability
                )
    ...
    def get_average_parameter_change(cls, state_dict_1, state_dict_2):
        prefixes = {
            ...
            if key.startswith("sub_policies")
        }
    ...
    def on_epoch_start(self, trainer, pl_module):
        self.policy_model_state_dict_epoch_start = copy.deepcopy(
            pl_module.policy_model.state_dict()
        )
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        log.info(
            f"Average Parameter change at epoch {trainer.current_epoch}: {average_parameter_change:.6f}."
        )
